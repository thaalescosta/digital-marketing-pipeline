...
from .dimensions import VideoEntity
from .distributions import youtube_daily_views
from .schemas import YoutubeVideoDailyRow
...
def generate_youtube_video_daily(
    ...
    for video in videos:
        if day < video.published_at:
            continue
        views = youtube_daily_views(
            rng, video.published_at, day, _base_popularity(video.video_id)
        )
