...
.DEFAULT_GOAL := help
.PHONY: help build test check lint clean generate plugin install-deps spec-lint spec-judge spec-scorer opencode
...
check: ## Drift check — tests + generators in --check mode (fails on drift)
	...
	@python3 scripts/generate-opencode.py --check

generate: ## Regenerate agent-router and OpenCode artifacts
	@python3 scripts/generate-agent-router.py
	@python3 scripts/generate-opencode.py

opencode: ## Regenerate OpenCode artifacts (.opencode/agents, commands)
	@python3 scripts/generate-opencode.py
