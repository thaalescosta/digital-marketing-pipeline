...
Per-agent model tiers live in each agent's frontmatter (authoritative) and are mirrored into `.claude/skills/agent-router/routing.json` at build time. This document no longer restates them — a previous copy here had drifted from the real values. Tiering rationale: pick the cheapest model that does the phase's job; escalate tier only where the phase demands depth.
