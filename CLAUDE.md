...
**What is AgentSpec?** A Claude Code plugin that provides structured AI-assisted development through a 5-phase SDD workflow, specialized for data engineering with 58 agents, 31 commands, 24 KB domains, and 21 skills (16 distributed in the plugin + 5 repo-local).
...
**Current Status:** v3.6.0 shipped (trust layer V0 for the built plugin, spec-driven agent creation pipeline + agent-architect, artifact scoring layer, `github-review-pr` repo-local review skill, single-source `plugin.json` version and release-branch flow). Linear is the project tracker (source of truth).
...
│   │   │                    #   GitHub trio (github-cr-adr/-issue, github-post-issue) + github-review-pr*,
...
│   └── scripts/             # init-workspace.sh
│
├── .opencode/               # OpenCode integration (generated from .claude/)
│   ├── agents/              # 59 flat subagents for @agent invocation
│   └── commands/            # 31 flat slash commands for /command execution
├── opencode.json            # OpenCode config (schema, instructions, skills discovery)
├── AGENTS.md                # Universal multi-agent instruction guide
...
│   ├── generate-opencode.py      # Regenerates .opencode/ agents and commands from .claude/
...
| `.claude/skills/github-review-pr/` | Pull-request review protocol (repo-local): ground in the linked issue/ADR, stateless blind + verifier reviewer roles, suites run at the PR head in a throwaway worktree, one structured review comment for a human to post |
