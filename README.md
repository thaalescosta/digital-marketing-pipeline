...
[![OpenCode Supported](https://img.shields.io/badge/OpenCode-Supported-orange?style=flat-square)](AGENTS.md)
...
[Install](#install) · [OpenCode](#using-with-opencode) · [Quick Start](#quick-start) · [Commands](#which-command-do-i-need) · [Agents](#58-agents-across-8-categories) · [Docs](docs/)
...
AgentSpec provides native first-class support for [OpenCode](https://opencode.ai) through `opencode.json`, `AGENTS.md`, and `.opencode/`:

1. **Zero Configuration**: If working in this repo or copying `.opencode/`, OpenCode automatically detects all 58 agents and 31 slash commands.
2. **Commands**: Type `/pipeline`, `/schema`, `/brainstorm`, `/define`, etc. OpenCode automatically routes the command to the appropriate specialist agent.
3. **Subagents**: Mention `@pipeline-architect`, `@schema-designer`, etc., directly in your conversation.
4. **Works with Free Models**: OpenCode agents are unbundled from specific proprietary models, allowing you to use OpenCode's free model limits (Gemini 2.5 Flash, Groq, Ollama, etc.) seamlessly.

To copy AgentSpec into an existing project for OpenCode:
```bash
cp -r .opencode opencode.json AGENTS.md your-project/
# Skills and knowledge base are shared
cp -r .claude/skills .claude/kb your-project/.claude/
```
